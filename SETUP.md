...
1. Copying the composer configuration file : 
    ...
    ```sh
    cp api/app/composer.json.dist api/app/composer.json
    ```
...
```sh
sudo docker run --rm -v $(pwd)/api/app:/app composer require symfony/yaml
```

### Updating PHP dependencies with composer

```sh
sudo docker run --rm -v $(pwd)/api/app:/app composer update
```
