...
init:
	@$(shell cp -n $(shell pwd)/api/app/composer.json.dist $(shell pwd)/api/app/composer.json 2> /dev/null)
...
clean:
	...
	@rm -Rf api/app/vendor
	@rm -Rf api/app/composer.lock
	@rm -Rf api/app/doc
	@rm -Rf api/app/report
	@rm -Rf etc/ssl/*
...
resetOwner:
	@$(shell chown -Rf $(SUDO_USER):$(shell id -g -n $(SUDO_USER)) $(MYSQL_DUMPS_DIR) "$(shell pwd)/etc/ssl" "$(shell pwd)/api/app" 2> /dev/null)
